...
if ($_SESSION['user'] != NULL) {
    ...
    $sql = "SELECT * FROM notifications where to_user='$user_id' and isboth_friends=0";
...
?>
    ...
    <body class="skin-light">
        <nav class="navbar navbar-expand-sm navbar-dark bg-primary sticky-top scrolling-navbar">
            ...
            <a href="#" class="navbar-brand"><i class="fab fa-facebook"></i> <b>RGUBOOK | Friend Requests</b> </a>
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-7">
                            <div class="friend_request_container">
                                ...
                                ?>
                                        <div class="card friend-request-card mt-2" style="color: black;">
                                            <div class="card-body">
                                                <div>Friend request from <strong><?php echo $notification_row['from_user']; ?></strong></div>
                                                <div>
                                                    <span><button data-notifid="<?php echo $notification_row['notification_id']; ?>" data-fromuser="<?php echo $notification_row['from_user']; ?>" class="btn btn-outline-default btn-sm p-1 waves-effect confirm">Confirm</button></span>
                                                    <span><button data-notifid="<?php echo $notification_row['notification_id']; ?>" data-fromuser="<?php echo $notification_row['from_user']; ?>" class="btn btn-outline-danger btn-sm p-1 waves-effect delete_request">Delete request</button></span>
                                                </div>
                                            </div>
                                        </div>
                                <?php }
                                } else {
                                    echo '<div class="card friend-request-card text-center" style="margin: 50px auto;padding: 20px;"><strong>No Friend Requests</strong></div>';
                                }
                                ?>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).on('click', '.confirm', function() {
            ...
            $.ajax({
                url: 'confirm.php',
                type: 'post',
                data: 'notif_id=' + notif_id + '&from_user=' + from_user,
                success: function(data) {
                    $('.friend_request_container').html(data);
                    toastr.success('You and ' + from_user + ' are now friends');
                }
            })
        });
        ...
        $(document).on('click', '.delete_request', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('.friend_request_container').html(data);
                    toastr.success(from_user + ' friends request successfully deleted!!!');
                }
            })
        });
    </script>
